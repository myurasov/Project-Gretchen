...
exports = {
  ...
  /**
   * Get screen width for the current orientation
   */
  ...
  /**
   * Fit one rect into another
   */
  fitIntoRect: function (w, h, maxW, maxH) {
    var h = h,
      w = w,
      scale = 1;
   ...
   * Get remote file
   */
  getRemoteFile: function (options) {
    ...
    x.setTimeout(options.timeout);

    x.onload = function () {
      if (x.status == 200) {
        options.success(this.responseData);
      } else {
        options.error("HTTP error " + x.status);
      }
    };
